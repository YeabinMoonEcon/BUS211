...
def year_finder(input):
    year_pattern = re.compile(r'\b\d{4}\b')
    match = re.search(year_pattern, input)
    if match:
...
class PresidentGuesser:
    ...
    def __call__(self, question):
        ...
        candidates = []
        question_date = year_finder(question)

        for president, (start_date, stop_date) in self._lookup.items():    
            if start_date.tm_year <= question_date <= stop_date.tm_year:
