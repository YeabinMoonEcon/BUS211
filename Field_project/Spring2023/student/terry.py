...
class PresidentGuesser:
    def train(self, training_data):
        self._lookup = {} # empty dictionary
        
        for row in training_data: # for every dictionary in training_data
            self._lookup[row["name"]] = (time.strptime(str(row["start"]), "%Y"), time.strptime(str(row["stop"]), "%Y"))
            # using [] to get access to element in list, dictionary...etc
            # using name as a key and a tuple (start, stop) as value
            # for example: "George Washington" : (start, stop)
    ...
    def __call__(self, question):
        ...
        for name, (start, stop) in self._lookup.items():
            if start <= time.strptime(question[-5: -1], "%Y") <= stop:
